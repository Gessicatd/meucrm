...
export default function PipelinesPage() {
  ...
  const handleDealMoved = useCallback(
    async (dealId: string, newStageId: string) => {
      ...
      const { error } = await supabase
        ...
        .eq("id", dealId);
      if (error) {
        toast.error(t('pipelines.toastMoveFailed'));
        refreshDeals();
      }
    },
  ...
  async function handleCreatePipeline() {
    ...
    if (error || !pipeline) {
      toast.error(t('pipelines.toastCreateFailed'));
      setCreating(false);
      return;
    }
    ...
    setCreating(false);
    toast.success(t('pipelines.toastCreated'));
  }
    ...
    <div className="space-y-6">
      ...
      <Dialog open={newPipelineOpen} onOpenChange={setNewPipelineOpen}>
        <DialogContent className="sm:max-w-sm bg-popover border-border">
          <DialogHeader>
            <DialogTitle className="text-popover-foreground">{t('pipelines.newPipelineTitle')}</DialogTitle>
          </DialogHeader>
          <div className="py-2">
            <Label className="text-muted-foreground">{t('pipelines.pipelineName')}</Label>
            <Input
